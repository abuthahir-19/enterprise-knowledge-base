...
export default function ArticleCard({ article, showStatus = false }) {
  ...
  return (
    <Link to={`/articles/${id}`}>
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 hover:shadow-md hover:border-blue-200 transition-all duration-200 h-full flex flex-col">
        {/* Header */}
            ...
            <div className="flex items-center gap-3">
              {author && (
                <span className="flex items-center gap-1">
                  <User size={12} />
                  {author.full_name || author.name || 'Unknown'}
                </span>
              )}
              {category && (
                <span className="flex items-center gap-1">
